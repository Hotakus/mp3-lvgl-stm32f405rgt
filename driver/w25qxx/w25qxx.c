...
static SPI_HandleTypeDef h_w25qxx_spi;
...
static void w25qxx_gpio(void)
{
    ...
    h_w25qxx_spi.Instance = W25QXX_SPI;
}
...
{
    return spi_full_duplex_trans_byte(&h_w25qxx_spi, byte);
}
...
void w25qxx_get_JEDECID(void)
{
    ...
    // W25QXX_CS_CLR;
    // jedecid_buf[0] = w25qxx_trans_byte( W25QXX_CMD_JEDECID );
    // HAL_SPI_Receive( &h_w25qxx_spi, jedecid_buf+1, 2, 0xFFFFFFFF );
    // DEBUG_PRINT( "JEDECID : 0x%X\n", jedecid_buf[0] );
    // DEBUG_PRINT( "JEDECID : 0x%X\n", jedecid_buf[1]  );
    // DEBUG_PRINT( "JEDECID : 0x%X\n", jedecid_buf[2]  );
    // W25QXX_CS_SET;

    // w25qxx.JEDECID  = jedecid_buf[0]<<16;
    // w25qxx.JEDECID |= jedecid_buf[1]<<8;
    // w25qxx.JEDECID |= jedecid_buf[2];

    // jedecid_buf[0] = W25QXX_CMD_JEDECID;
    // HAL_SPI_Transmit( &h_w25qxx_spi, jedecid_buf, 1, 0xFFFFFFFF );
    // HAL_SPI_Receive( &h_w25qxx_spi, jedecid_buf, 3, 0xFFFFFFFF );
    // W25QXX_CS_SET;
 ...
 * @return w25qxx_stat
 ************************************************/
w25qxx_stat w25qxx_read_sector(uint8_t* rec_buf,
    uint32_t sector,
    uint32_t cnt)
{
 ...
 * @return w25qxx_stat
 ************************************************/
w25qxx_stat w25qxx_writ_sector(uint8_t* send_buf,
    uint32_t sector,
    uint32_t cnt)
{
