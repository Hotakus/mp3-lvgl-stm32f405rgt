...
void mp3_analyse_frame_header(mp3_info_t* mp3)
{
  ...
  DEBUG_PRINT("version %d\n", mp3->fh.bitrate_index);

  DEBUG_PRINT("mp3->bitrates %d\n", mp3->bitrates);
  DEBUG_PRINT("mp3->file_size %d\n", mp3->file_size);
  DEBUG_PRINT("mp3->file_size_splited %d\n", mp3->file_size_splited);
...
void mp3_analyse_bitrate(mp3_info_t* mp3)
{
  ...
  if (mp3->fh.version == 0x03) {
    ...
    if (mp3->fh.layer == 0x03) {
      ...
      if (mp3->fh.bitrate_index <= 4 && mp3->fh.bitrate_index >= 1) {
        base = 32;
        inc_var = 8;
        mp3->bitrates = base + (inc_var * (mp3->fh.bitrate_index + 3));
      } else if (mp3->fh.bitrate_index > 4 && mp3->fh.bitrate_index <= 9) {
        base = 56;
        inc_var = 16;
        mp3->bitrates = base + (inc_var * (mp3->fh.bitrate_index + 1));
      } else if (mp3->fh.bitrate_index > 9 && mp3->fh.bitrate_index < 14) {
        base = 128;
        inc_var = 32;
        mp3->bitrates = base + (inc_var * (mp3->fh.bitrate_index - 5));
      } else if (mp3->fh.bitrate_index == 14) {
        mp3->bitrates = 320;
      }
    }
